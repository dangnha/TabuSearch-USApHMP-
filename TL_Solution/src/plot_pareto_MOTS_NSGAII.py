...
import os
from src.data import initial_coords
import re
...
def read_box_plot_data(filename):
    ...
    for line in content:
        if line.startswith('Pareto Frontier(n='):
            ...
            cost_line = next((l for l in content[content.index(line)+1:] if 'Cost of solution:' in l), None)
            time_line = next((l for l in content[content.index(line)+1:] if 'Time of solution:' in l), None)
        ...
        if match:
            ...
            if pareto_match:
                ...
                for solution in pareto_front:
                    ...
                    # plt.text(time_metric, cost/100, str(hubs), fontsize=14, ha='left', color='darkblue')
                ...
                if (n,p) in box_plot_solutions:
                    for cost, time, hubs in box_plot_solutions[(n,p)]:
                        ...
                        # plt.text(time, cost, str(hubs), fontsize=14, ha='right', color='darkgreen')
